<?php
	...
	function format_date($id,$tgl,$lang,$type=null) {
		$arr2= explode(' ',$tgl);
		$arr = explode('-',$arr2[0]);
	...
	function format_tanggal_indonesia($tgl){
		$arr2= explode(' ',$tgl);
		$arr = explode('-',$arr2[0]);
		$hari= switch_day(date('N',strtotime($tgl)));
		$result = $hari.', '.$arr[2].' '.switch_month($arr[1],'id').' '.$arr[0];
	...
	function get_image($img){
		if($img !=''){
			...
			$result = 'assets/images/image-not-found.png';
		}

		return $result;
	}

	function url_regex($str){
